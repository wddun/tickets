...
- **Self-service ticket returns**: an attendee who can't come hands their own ticket back rather than emailing the organiser to be taken off the list. Off per event until `events.ticketReturnsEnabled` is set (`PUT /api/event/:id/ticket-returns`, `manage_event`). **A return is an expiry, not a new kind of void** — it goes through the same `expireTicket()` with `{returnedByAttendee: true}`, so the freed seat, the voided Wallet pass, `isTicketExpired()`'s refusal at the door and the waitlist promotion are all the existing, already-tested behaviour; the extra `tickets.returnedAt` stamp only records *who* did it, which is how the dashboard says "Returned" rather than "Expired" and how `ticket.returned` is distinguished in the audit log. Un-expiring clears `returnedAt` too, so a reinstated ticket isn't left labelled as given back. Both attendee routes (`GET /api/registration/:id/manage`, `POST /api/registration/:id/return`) are session-less, on the same trust model as the waitlist's own status page: the `registrationId` is a nanoid known only to the person it was emailed to, and already acts as a bearer token for that registration's Wallet passes. They deliberately **never return ticket QR tokens** — a manage link must not double as a scannable ticket. The link is appended to the confirmation email *outside* the block template (like an unsubscribe link), because an organiser who enables returns but has a saved custom template would otherwise advertise a feature nobody can reach.
- **Return cutoff and refunds.** The cutoff is stored as `events.ticketReturnCutoffMinutes` with `ticketReturnCutoffUnit` alongside it purely for redisplay — minutes are the one comparable number, so "2 days" and "48 hours" are the same cutoff, and the unit never takes part in deciding whether the window is open (`ticketReturnWindow()`). 0 keeps returns open to the start time; an event with no start time never closes them. `events.ticketReturnRefund` is `'none'` or `'auto'` and is reported as `'none'` on a free event whatever is stored — there is no approval queue by design, the organiser chooses once. `refundReturnedTickets()` **never throws and never rolls the return back**: by the time it runs the seat is released and the waitlist may already have moved, and re-seating someone who has said they aren't coming is worse than a refund that needs chasing. It refunds the returned tickets' share of the order (the whole remainder once nothing in the registration is still live, so rounding can't strand a few cents) and reports `refunded` / `failed` / `unavailable` / `nothing_to_refund` honestly — the attendee is never told they were refunded when they weren't.
