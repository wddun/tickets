...
- **Service worker**: `public/sw.js` serves everything in `PRECACHE` (`scanner.html`, `checkin.html`, …) **cache-first** so the door works offline. An installed scanner keeps running the copy it already has until `CACHE` changes — so bump `CACHE` whenever a precached file changes, or the deploy will not reach existing installs.
