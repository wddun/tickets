...
window.HELP_CONTENT = {
    ...
    'general.ticketExpiresAt': {
        ...
        details: [
            `Which not-checked-in tickets this cutoff actually reaches is set by <strong>Which Tickets This Expires</strong> below — by default it only fires when doing so would free a seat for someone on the waitlist, but it can also be set to a hard, unconditional cutoff.`,
    ...
    'general.ticketExpiryMode': {
        title: 'Which Tickets This Expires',
        summary: 'Controls how far the cutoff above reaches: only enough to free waitlist seats, everyone once the waitlist is on, or everyone always.',
        details: [
            `<strong>Only enough to free seats for the waitlist</strong> (default) — the cutoff only does anything if the event actually needs it: the waitlist must be enabled, at least one person must still be waiting, and the event must be full. This exists purely to free seats for a waitlist, so it never expires a ticket nobody's waiting for. As the waitlist empties out (everyone still waiting gets promoted), the cutoff naturally stops expiring further tickets; adding more people to the waitlist lets it resume.`,
            `<strong>Everyone not checked in, if the waitlist is on</strong> — every not-checked-in ticket expires once the cutoff hits, as long as the waitlist feature is switched on for this event, whether or not anyone is actually waiting right now or the event is full.`,
            `<strong>Everyone not checked in, always</strong> — a hard, unconditional cutoff. Every not-checked-in ticket expires the moment it's reached, regardless of the waitlist or capacity. Use this for something like a same-day giveaway where an unclaimed ticket should just stop working at a fixed time.`,
            `Whichever mode is chosen, whether a freed seat is then offered to the waitlist is still governed separately by <strong>Promote the waitlist when tickets expire</strong> below.`,
        ],
    },
